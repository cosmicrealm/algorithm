...
class GroupQueryAttention(torch.nn.Module):
    ...
    def forward(self, hidden_state, attention_mask=None):
        ...
        print('1:q:',query.shape,'k:',key.shape,'v:',value.shape,)
        
        query = self.split_head(query)
        key = self.split_head(key, self.group_num)
        value = self.split_head(value, self.group_num)
        
        print('2:q:',query.shape,'k:',key.shape,'v:',value.shape,)
...
def test_gqa():
    ...
    model = GroupQueryAttention(hidden_size, num_heads, group_num)
